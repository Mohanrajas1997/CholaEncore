...
Public Class frmmicrmaster
    ...
    Private Sub Importexcel(ByVal FilePath As String, ByVal SheetName As String)
        ...
        lsFldNmesInfo = "SNo|MICR Code|Bank Name|Bank Branch|Micr Centre "
        ...
        Try
            ...
            For liIndex As Integer = 0 To lExcelDatatable.Rows.Count - 1
                ...
                If lExcelDatatable.Rows(liIndex).Item("MICR Code").ToString.Trim = "" Then
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("MICR Code").ToString.Trim.Length <> 9 Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = " Invalid MICR Code: " & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CODE").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                'Check for Duplicate
                lssql = " Select micr_gid from chola_mst_tmicr where 1=1 and " & _
                            " micr_code='" & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR Code").ToString) & "'"
                ...
                If Val(lsFileGid) > 0 Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = " Duplicate exist " & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CODE").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If
                ...
                Try
                    ...
                    lssql = " Insert into chola_mst_tmicr(micr_file_gid,micr_code,micr_bankname,micr_bankbranch,micr_micrcentre)"
                    lssql &= " values ("
                    lssql &= lsFileMstGid & ","
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR Code").ToString) & "',"
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("Bank Name").ToString) & "',"
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("Bank Branch").ToString) & "',"
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("Micr Centre").ToString) & "')"
