...
Public Class frmbounceimport
    ...
    Private Sub Importexcel(ByVal FilePath As String, ByVal SheetName As String)
        ...
        Dim lsReasonGid As String


        lsFldNmesInfo = "Return date|Nbr Micr|Amt Chk|Chq Date|Retn Rsn|AWB No|Agreement no|"
        ...
        Try
            ...
            For liIndex As Integer = 0 To lExcelDatatable.Rows.Count - 1
                ...
                If lExcelDatatable.Rows(liIndex).Item("Return date").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "Return Date Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("Nbr Micr").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "Cheque No Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If


                If lExcelDatatable.Rows(liIndex).Item("Chq Date").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "Chq Date Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("Amt Chk").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "Cheque Amount Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("Retn Rsn").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "Return Reason Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("AWB No").ToString.Trim = "" Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = "AWB No Blank : " & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If


                lssql = " Select reason_gid from chola_mst_tbouncereason where reason_deleteflag='N' and " & _
                            " reason_name='" & QuoteFilter(lExcelDatatable.Rows(liIndex).Item("Retn Rsn").ToString.Trim) & "'"

                lsReasonGid = gfExecuteScalar(lssql, gOdbcConn)

                'Write Dulicate Record in Error log
                If Val(lsReasonGid) = 0 Then
                    lssql = ""
                    lssql &= " insert into chola_mst_tbouncereason ("
                    lssql &= " reason_name,reason_insertdate,reason_insertby)"
                    lssql &= " values ("
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex).Item("Retn Rsn").ToString.Trim) & "',"
                ...
                Try
                    ...
                    lssql = " Insert into chola_trn_tbounce (bounce_file_gid,bounce_returndate,bounce_chqno,bounce_chqdate,"
                    lssql &= " bounce_chqamount,bounce_reason_gid,bounce_awbno,bounce_agreementno)"
                    lssql &= " values ("
                    lssql &= lsFileMstGid & ","
                    lssql &= "'" & Format(CDate(QuoteFilter(lExcelDatatable.Rows(liIndex)("Return date").ToString)), "yyyy-MM-dd") & "',"
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("Nbr Micr").ToString) & "',"
                    lssql &= "'" & Format(CDate(QuoteFilter(lExcelDatatable.Rows(liIndex)("Chq Date").ToString)), "yyyy-MM-dd") & "',"
                    lssql &= "" & Val(QuoteFilter(lExcelDatatable.Rows(liIndex)("Amt Chk").ToString)) & ","
                    lssql &= "" & lsReasonGid & ","
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("AWB No").ToString) & "',"
...
GoNext:
    ...
    Private Sub frmbounceimport_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Enter Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub frmbounceimport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        lblTotal.Visible = False
        Me.KeyPreview = True
        txtFileName.Focus()
        txtFileName.Text = ""
