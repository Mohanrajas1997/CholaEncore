...
Public Class frmcitymaster
    ...
    Private Sub Importexcel(ByVal FilePath As String, ByVal SheetName As String)
        ...
        lsFldNmesInfo = "SNo|MICR CITY CODE|MICR CITY NAME| "
        ...
        Try
            ...
            For liIndex As Integer = 0 To lExcelDatatable.Rows.Count - 1
                ...
                If lExcelDatatable.Rows(liIndex).Item("MICR CITY CODE").ToString.Trim = "" Then
                    GoTo GoNext
                End If

                If lExcelDatatable.Rows(liIndex).Item("MICR CITY CODE").ToString.Trim.Length <> 3 Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = " Invalid MICR Code: " & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CITY CODE").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If

                'Check for Duplicate
                lssql = " Select city_gid from chola_mst_tcity where 1=1 and " & _
                            " city_micrcode='" & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CITY CODE").ToString) & "'"
                ...
                If Val(lsFileGid) > 0 Then
                    lidup += 1
                    liIsDuplicate += 1
                    lsDuplicatedtl = " Duplicate exist " & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CITY CODE").ToString) & ""
                    PrintLine(1, lsDuplicatedtl)
                    GoTo GoNext
                End If
                ...
                Try
                    ...
                    lssql = " Insert into chola_mst_tcity(city_file_gid,city_micrcode,city_name)"
                    lssql &= " values ("
                    lssql &= lsFileMstGid & ","
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CITY CODE").ToString) & "',"
                    lssql &= "'" & QuoteFilter(lExcelDatatable.Rows(liIndex)("MICR CITY NAME").ToString) & "')"
